...
### R253 补记：为未决项补齐任务卡（交接可执行性）

**发现的交接缺陷**：`tasks/` 下的卡止于 **G76**，**G77–G84 都没有卡**——而 `AGENTS.md` 要求 Worker 动手前必须读**自己的任务卡**。于是「按交接节的推荐去做 G81」这一步**没有可读的规格**：信息只存在于本文件的 GAP 条目里，**不在工作流要求的载体里**。

**已补（编排者产出，非代码）**：
- `tasks/EVO-G81-export-reconcilable.md`（P1，最高价值）：要求导出物**行和 == 合计**、含**可与 `logs` 对齐的标识列**（只能追加、前 15 列逐字不变）、**任何标签值都不能改变 CSV 行结构**；含产物级验收、错误场景（长/引号/逗号/CRLF/空库）与契约同步要求。
- `tasks/EVO-G83-notices-consistency.md`（P2）：**同一件事不应只在某一个命令里被说明**（共享库提示只在 `summary`；`--tag` 生效但 `logs` 看不见标签）。含「本该静默就说明理由，不要为对齐而加噪声」的取向约束。
...
## EVO-G91 交付（R261）——trends 金额单元格的页面级断言 + 金额 6 位精度页面级取证

**一句话**：把 G90 在 `overview` 上做过的三件事（**单元格锚点** / **真价差 fixture** / **「fixture 真的带区间吗」守卫**）在 `trends` 的「区间成本」格上再做一遍，并让**同一条 fixture** 顺带证明「金额 6 位精度」在页面上真的印出来了。

**盲区（开工前实读确认）**：`scripts/e2e/run.mjs` 里 `trends` 字面量**零命中** ⇒ overview 那格被锚定之后，`trends` 那格**即使回去读 G89 刚劝宿主别读的 `costUsd`，e2e 照绿**。BACKLOG ⑥ 因此只闭合了一半——本片关掉另一半。

**改动（2 个代码文件）**：`apps/dashboard/components/views/trends.tsx`（`testId="trends-cost-span"`，复用 G90 已建好的 `StatCard.testId` 可选参数）；`scripts/e2e/run.mjs`（新增微余数行 + 两条**非空洞守卫** + `/trends` 单元格断言 + 与 overview 单元格的**交叉校验** + PASS 取数）。**`packages/mik/**` 零改动**（`git status --porcelain -- packages/mik` 为空）。

**两次红跑（本片核心证据，均逐字节还原）**：
1. 把取值临时改回点估计 → `FAIL  the trends money cell prints "$0.036646", expected the recorded band "$0.034301 ~ $0.044301"`，`exit 1`（elapsed 58.5 s）；
2. **只删锚点**（M3 建议 ① 的补证）→ `FAIL  the trends money cell has no data-testid="trends-cost-span" anchor — the DASH check cannot tell which cell it is reading`，`exit 1`。
   还原以 **SHA256 为证**：`trends.tsx` 与整份 `git diff` 的哈希前后一致 ⇒ 被评审、被提交的是同一份产物。

**为什么必须加那条微余数行**：既有 fixture 的 low/high 合计是 **34300 µ$**（`%100 === 0`）⇒ `formatUsd` 走 **4 位**分支 ⇒ 「6 位精度」在**页面上没有可观测后果**。加 **1 微**后合计变 `34301`/`44301` ⇒ `$0.034301 ~ $0.044301`：**一行同时给出「真区间」与「6 位」**，且两条守卫（`micros % 100 !== 0`、期望串匹配 `\$\d+\.\d{6}`）保证 fixture 退化时断言会红而不是空洞。

**门禁**：两侧 `tsc --noEmit` 0；`mik` **694 例 / 39 文件**（`LC_ALL=zh_CN.UTF-8` 与 `en_US.UTF-8` **各一次**，全程**未设 `MIK_LANG`**）；看板 **15/15**；`node scripts/e2e/run.mjs` **exit 0**（DASH 单元格读 `$0.034301 ~ $0.044301`）。**独立评审 PASS（置信度 0.75）**：`modelinfra-mission-02\evidence\M3-verdict.md`。

**状态**：本地**已提交**，**未 push、未升版、未发布**——发布是人类闸门（本片与 G90 一样运行时零变化；若发版，须先重做 G36）。

**评审裁定的一处交付偏差**：实现者申报「实测 base = **0.0162**」，经指挥与独立评审各自算出应为 **0.0243**（漏掉 EX1 的加倍行 `0.0036` 与 AC7 用 P2 价的新请求 `0.0081`；`0.0243 + 0.012345 + 0.000001 = 0.036646` 与红跑打印值逐位吻合）。**实害为零**（正因算错，它没有把该数字钉进守卫，只钉了规则），但属 R258 同类：**「实测」二字不能用在没被实测的数字上**。

**未关闭（登记，不是待办）**：① `trends.tsx:63`「最贵的一天」读 `costUsd`，且 `busiest` 是**按点估计排序**的（29–32 行）；② `trends.tsx:104` 按天明细的成本列读 `costUsd`——两者同族、相邻，本片**只记录**；③ `/trends` 的**离线态**（`--break-dashboard`）未加断言；④ 无机械断言「其他单元格不带 `testId`」（只有 grep）。

**方法学留痕（两条）**：
- **独立评审通道两次失败**：M3 前两次派单（`410a0940`、`29150b34`）都在 ~60 s / ≤9 步后**无最终消息即失败**，`contextPressure = 0`（排除上下文超限；对照组 M2 为 90 步 / 19 分钟）。第三次改成**极小载荷、有界步数**后成功。⇒ **短任务早停与"上下文爆掉"是两种不同故障**，诊断要看会话统计而不是猜。
- **预算偏差（如实登记）**：立项时写 `next build ≤ 1 次`，实际用了 **3 次**（M2 两次 + 定向红跑一次）——改 `trends.tsx` 会必然触发看板重建，这个上限当时定错了。
