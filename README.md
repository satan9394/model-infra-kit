...
> **版本**：当前 `latest` 是 **0.3.1**（与 `0.3.0` 代码相同）。**先读 [`CHANGELOG.md`](./CHANGELOG.md)**——它把已有的 41 个版本压成 5 段、逐版一行，并写清了「为什么会有这么多版本」。**规则已收窄**：只有当**已发布包的行为或公共接口发生变化**时才升版发布，验证片/文档片只提交不发版（`0.1.x`–`0.3.1` 那 41 个版本是旧规则的产物）。
