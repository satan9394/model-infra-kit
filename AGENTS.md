...
## 当前状态（R197 校正——本节的版本/基线数字必须随收口更新）

- **版本 v0.2.18**，npm 与 GitHub Release 均已发布；`main` 分支 CI（ubuntu/macos/windows 三 OS）全绿。
- **测试基线 572 例**（`pnpm --filter model-infra-kit test`，28 个文件），`tsc --noEmit` 0 错误，`node scripts/e2e/run.mjs` exit 0，`node scripts/check-envs.mjs` 三环境 PASS，字典 **299/299**。**locale 相关改动请在 `LC_ALL=en_US.UTF-8` 下复跑一遍**，本机 zh-CN 会掩盖 en 侧缺陷。**改 `package.json` 版本后必须重跑全量**（R99）。**涉及平台/分块/外部命令的测试必须以 CI 为准**（R187/R193）。
- **已验收发布的切片**：**G01–G72**（`0.1.7` → `0.2.18`）。产品演进全貌、GAP_MAP（**G01–G77**）、每轮验收留痕，见 **`docs/product-evolution.md`**（编排者维护的唯一权威状态文件，**比本节更新更快，冲突时以它为准**）。
